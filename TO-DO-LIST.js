...
function adding_column(new_colum_name){
    ...
    colum_state()
    notification(`Se creo la columna ${new_colum_name}`,'success')
}
...
function add_task(call_back) {
    ...
    call_back()
    notification(`Se creo la tarea ${new_task.tittle}`,'success')
}
...
function notification(message,type){
    ...
    const tool_tip_text=document.createElement("p")
    tool_tip_text.setAttribute("class","tool_tip_text")
    tool_tip_text.innerText=message
    const icon=document.createElement("svg")
    switch (type) {
        case 'warning':
            icon.innerHTML=`
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="white" class="bi bi-exclamation-triangle" viewBox="0 0 16 16">
        ...
        case 'success':
            icon.innerHTML=`
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="white" class="bi bi-check-circle" viewBox="0 0 16 16">
    ...
    tool_tip.appendChild(icon)
    tool_tip.appendChild(tool_tip_text)
    container.appendChild(tool_tip)
    setTimeout(()=>{
        container.removeChild(tool_tip)
    },4000)
}
